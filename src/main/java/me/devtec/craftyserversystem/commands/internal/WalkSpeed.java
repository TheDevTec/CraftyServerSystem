		...
		if (sendMessage)
			if (!sender.equals(target)) {
				TextRenderer placeholders = renderer().placeholder("sender", sender.getName())
						.placeholder("target", target.getName()).placeholder("value", Math.max(0, Math.min(10, speed)));
				msg(target, "other.target", placeholders);
				msg(sender, "other.sender", placeholders);
			} else
				msg(target, "self", renderer().placeholder("target", target.getName()).placeholder("value",
