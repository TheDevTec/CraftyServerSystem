...
public class SetHome extends CssCommand {
	...
	public void register() {
		...
		CommandStructure<Player> cmd = CommandStructure.create(Player.class, P_DEFAULT_PERMS_CHECKER, (sender, structure, args) -> {
			...
			msg(sender, "set", renderer().placeholder("home", "home").placeholder("x", pos.getX()).placeholder("y", pos.getY()).placeholder("z", pos.getZ()).placeholder("yaw", pos.getYaw())
		...
		cmd.argument(null, 1, (sender, structure, args) -> {
			...
			if(homes.size() >= maxHomes) {
				msg(sender, "overlimit", renderer().placeholder("totalHomes", homes.size()).placeholder("maxHomes", maxHomes));
				return;
			}
			Position pos;
			HomeManager.get().setHome(sender.getName(), args[0].toLowerCase(), pos = Position.fromEntity(sender));
			msg(sender, "set", renderer().placeholder("home", args[0].toLowerCase()).placeholder("x", pos.getX()).placeholder("y", pos.getY()).placeholder("z", pos.getZ())
			        .placeholder("yaw", pos.getYaw()).placeholder("pitch", pos.getPitch()).placeholder("world", pos.getWorldName()));
