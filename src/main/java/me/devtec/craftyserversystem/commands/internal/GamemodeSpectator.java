...
public class GamemodeSpectator extends CssCommand {
	...
	public void register() {
		if(isRegistered() || Ref.isBefore(8, 0))
			return;
	...
	public void changeGameMode(Player target, boolean sendMessage, CommandSender sender) {
		if(sendMessage)
			if(target.equals(sender)) {
				TextRenderer placeholders = renderer().placeholder("target", target.getName()).placeholder("gamemode", "spectator");
				msg(sender, "self", placeholders);
			} else {
				TextRenderer placeholders = renderer().placeholder("target", target.getName()).placeholder("sender", sender.getName()).placeholder("gamemode", "spectator");
		...
		if(!Bukkit.isPrimaryThread())
			BukkitLoader.getNmsProvider().postToMainThread(() -> target.setGameMode(GameMode.SPECTATOR));
		else
